...
async def create_service_request(user_id: int, name: str, suggested_price: float, keywords: list, description: str) -> str:
    doc = {
        "user_id": user_id,
        "name": name,
...
async def get_user_daily_request_count(user_id: int) -> int:
    today = datetime.datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    return await db.service_requests.count_documents({
        "user_id": user_id,
        "created_at": {"$gte": today}
    })
...
async def get_service_request(request_id: str):
    from bson import ObjectId
    try:
        return await db.service_requests.find_one({"_id": ObjectId(request_id)})
    except Exception:
        return None
